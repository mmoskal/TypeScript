...
module ts.thumb {
    ...
    class Line {
        ...
        public singleReg() {
            ts.Debug.assert(this.getOp() == "push" || this.getOp() == "pop")
            var k = 0;
            var ret = -1;
    ...
    export class File {
        ...
        private emitShort(op: number) {
            ts.Debug.assert(0 <= op && op <= 0xffff);
            this.buf.push(op);
        }
        ...
        private align(n: number) {
            ts.Debug.assert(n == 2 || n == 4 || n == 8 || n == 16)
            ...
            for (var i = 0; i < mylines.length; ++i) {
                ...
                if (ln.type == "instruction") {
                    ...
                    if (lnop == "bb" && lb11.encode(ln.numArgs[0]) != null) {
                        ...
                        ts.Debug.assert(ln.numArgs[0] > 0)
                        ln.update("")
                        lnNext.update("")
        ...
        private peepPass() {
            ...
            this.labels = {};
            this.iterLines();
            ts.Debug.assert(!this.checkStack || this.stack == 0);
            this.finalEmit = true;
            this.iterLines();
    ...
    function expect(disasm: string) {
        ...
        if (b.errors.length > 0) {
            console.log(b.errors[0].message)
            ts.Debug.fail("ASMTEST: not expecting errors")
        }

        if (b.buf.length != exp.length)
            ts.Debug.fail("ASMTEST: wrong buf len")
        for (var i = 0; i < exp.length; ++i) {
            if (b.buf[i] != exp[i])
                ts.Debug.fail("ASMTEST: wrong buf content, exp:" + tohex(exp[i]) + ", got: " + tohex(b.buf[i]))
        }
    }
